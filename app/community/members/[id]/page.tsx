...
export default function MemberPublicProfilePage({ params }: { params: { id: string } }) {
    ...
    if (!profile) {
        return (
            <div className="min-h-screen bg-[#07111F] flex flex-col justify-center items-center text-gray-500">
                <FiAlertCircle className="w-12 h-12 mb-4 text-gray-600" />
                <p>User profile not found.</p>
                <Link href="/community" className="text-gold underline mt-2">Home</Link>
            </div>
        );
    }
    ...
    return (
        <div className={`min-h-screen transition-colors duration-300 py-16 px-4 md:px-8 ${theme === 'dark' ? 'bg-[#07111F] text-white' : 'bg-white text-dark-light'
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="glass-panel p-8 rounded-3xl border border-blue-500/20 relative overflow-hidden flex flex-col sm:flex-row items-center sm:items-start space-y-6 sm:space-y-0 sm:space-x-6 gap-6">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/10 blur-3xl rounded-full"></div>

                    <div className="w-24 h-24 bg-slate-900 border border-blue-500/20 rounded-full flex items-center justify-center font-bold text-3xl text-gold border border-blue-500/30 overflow-hidden shadow-xl">
                        {profile.avatar ? (
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    ...
                    <div className="md:col-span-2 space-y-8">
                        ...
                        <div className="glass-panel p-8 rounded-3xl space-y-3">
                            <h3 className="font-bold text-white text-md border-b border-blue-500/20 pb-2">{cur.bio}</h3>
                        ...
                        {/* Skills */}
                        <div className="glass-panel p-8 rounded-3xl space-y-3">
                            <h3 className="font-bold text-white text-md border-b border-blue-500/20 pb-2">{cur.skills}</h3>
                            {!profile.skills || profile.skills.length === 0 ? (
                                <p className="text-gray-500 text-xs italic">{cur.noSkills}</p>
                            ) : (
                                <div className="flex flex-wrap gap-2 pt-2">
                                    {profile.skills.map((skill) => (
                                        <span key={skill} className="bg-slate-900 border border-blue-500/20 border border-blue-500/20 text-gray-300 text-xs px-3 py-1.5 rounded-xl font-medium">
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {/* Badges */}
                        <div className="glass-panel p-8 rounded-3xl space-y-3">
                            <h3 className="font-bold text-white text-md border-b border-blue-500/20 pb-2">{cur.badges}</h3>
                            {!profile.badges || profile.badges.length === 0 ? (
                                ...
                                <div className="grid sm:grid-cols-2 gap-4 pt-2">
                                    {profile.badges.map((b) => (
                                        <div
                                            key={b._id}
                                            className="p-3 bg-slate-900 border border-blue-500/20 border-l-4 rounded-xl flex items-center justify-between"
                    ...
                    <div className="space-y-6">
                        <div className="glass-panel p-8 rounded-3xl space-y-6">
                            <h3 className="text-md font-bold border-b border-blue-500/20 pb-2">{cur.performance}</h3>

                            <div className="text-center py-4 bg-slate-900 border border-blue-500/20 rounded-2xl">
